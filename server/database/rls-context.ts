 ...
 */
export async function setUserContext(db: DatabaseWithSchema, userId: string): Promise<void> {
 ...
 */
export async function getCurrentUserContext(db: DatabaseWithSchema): Promise<string | null> {
  try {
...
export async function validateUserContext(
  ...
  if (!currentUserId) {
    throw new Error('No user context is set. Call setUserContext() before database operations.')
  }
  
  if (currentUserId !== expectedUserId) {
    throw new Error(`User context mismatch. Expected: ${expectedUserId}, Got: ${currentUserId}`)
  }
}
